...
export async function onRequestGet(context) {
  ...
  try {
    const rows = await env.DB.prepare(`
      SELECT
        CASE
          WHEN (fbc IS NOT NULL AND fbc != '')
            OR LOWER(utm_source) IN ('facebook','instagram','fb','meta','ig') THEN 'meta'
          WHEN (gclid IS NOT NULL AND gclid != '')
            OR (gbraid IS NOT NULL AND gbraid != '')
            OR (wbraid IS NOT NULL AND wbraid != '')
            OR LOWER(utm_source) IN ('google','google-ads','googleads','youtube') THEN 'google'
          ELSE 'organic'
        END as source_type,
        COUNT(*) as sales,
